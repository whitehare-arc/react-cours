...
export const calculatorReducer = (state, action) => {
  switch (action.type) {
    case 'SET_NUM1':
      return { ...state, num1: action.payload, displayValue: action.payload + (state.operator ? ` ${state.operator} ${state.num2}` : '') };
    case 'SET_NUM2':
      return { ...state, num2: action.payload, displayValue: state.num1 + ` ${state.operator} ${action.payload}` };
    case 'SET_OPERATOR':
      return { ...state, operator: action.payload, displayValue: state.num1 + ` ${action.payload}` };
    case 'CLEAR_ERROR':
      return { ...state, error: '' };
    case 'SET_ERROR':
